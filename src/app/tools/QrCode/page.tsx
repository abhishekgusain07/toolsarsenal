...
interface EditorSidebarProps {
  ...
  qrCodeBackgroundColor: string
  setQrCodeBackgroundColor: (value: string) => void
  qrCodeTextColor: string
  setQrCodeTextColor: (value: string) => void
}
  ...
  return (
        ...
        >
          ...
          <EditorSidebar 
            ...
            qrCodeBackgroundColor={qrCodeBackgroundColor}
            setQrCodeBackgroundColor={setQrCodeBackgroundColor}
            qrCodeTextColor={qrCodeTextColor}
      ...
      >
        ...
        >
            <QRCode
              ...
              fgColor={qrCodeTextColor}
              style={{ height: "auto", maxWidth: "100%", width: "100%", padding: "8px", backgroundColor: qrCodeBackgroundColor, borderRadius: "1px", borderTop: "none", borderLeft: "none" , borderRight: "none", borderImage: "initial", overflow: "hidden", color: qrCodeTextColor}}
              viewBox={'0 0 21 21'}
            />
   </div>
...
const EditorSidebar = ({ 
  ...
  const qrCodeTextColorRef = useRef<HTMLInputElement>(null)
  const qrCodeBackgroundColorRef = useRef<HTMLInputElement>(null)
            ...
            <>
              ...
              <div>
                <Input 
                    ...
                    label="Enter URL"
                    className="text-sm"
                    placeholder="e.g. https://toolsarsenal.vercel.app"
                    showResetIcon={true}
                    onResetIconPressed={handleReset}
                />
                <div className="flex flex-row justify-between">
                  <div className="flex items-center  justify-between cursor-pointer mt-4 relative py-0.5 px-3 border border-gray-300 dark:border-gray-600 rounded-2xl mr-4 w-[45%]"
                  onClick={() => qrCodeTextColorRef.current?.click()}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      qrCodeTextColorRef.current?.click();
                    }
                  }}
                  >
                    <div className="flex flex-row">
                    <span>Color</span>
                    <input type="color" value={qrCodeTextColor} onChange={(e) => setQrCodeTextColor(e.target.value)} className="w-0 size-[0.1px] bg-white"
                    ref={qrCodeTextColorRef}
                    />
                    </div>
                    <span className="h-4 w-4 rounded-full ml-1 border border-gray-400" style={{backgroundColor: "rgb(0, 0, 0)"}}/>
                  </div>
                  <div className="flex items-center w-[45%] cursor-pointer mt-4 relative py-0.5 px-3 border border-gray-300 dark:border-gray-600 rounded-2xl justify-between"
                    onClick={() => qrCodeBackgroundColorRef.current?.click()}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                        qrCodeBackgroundColorRef.current?.click();
                      }
                    }}
                  >
                    <div className="flex flex-row">
                    <span>Background</span>
                    <input type="color" value={qrCodeBackgroundColor} onChange={(e) => setQrCodeBackgroundColor(e.target.value)}
                    className="size-[0.1px] bg-white"
                    ref={qrCodeBackgroundColorRef}
                    />
                    </div>
